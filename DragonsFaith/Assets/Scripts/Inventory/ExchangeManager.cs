...
namespace Inventory
{
    ...
    public class ExchangeManager : NetworkBehaviour
    {
        ...
        private void SendItemResponseServerRpc(bool isSpaceAvailable)
        {
            ...
            Debug.Log(isSpaceAvailable
                ? "Item accepted, thanks client!"
                : "Item refused, Item accepted, sorry client!");
        ...
        public void NotifyHealToAnother(int heal)
        {
            if (IsHost)
            {
                NotifyHealClientRpc(heal);
            }
            else
            {
                NotifyHealServerRpc(heal);
            }
        }

        [ServerRpc(RequireOwnership = false)]
        private void NotifyHealServerRpc(int heal)
        {
            if (!IsHost) return;

            foreach (var player in FindObjectsOfType<PlayerGridMovement>())
            {
                if (player.GetTeam() == PlayerGridMovement.Team.Players &&
                    player.gameObject.GetComponent<NetworkObject>() != NetworkManager.Singleton.LocalClient.PlayerObject)
                {
                    player.GetComponent<CharacterInfo>().Heal(heal);
                }
            }
        }

        [ClientRpc]
        private void NotifyHealClientRpc(int heal)
        {
            if (IsHost) return;
